...
public class ReadExample {
    ...
    public static void main(String[] args) {
        ...
        ReadExample.readFileByBytes(fileName);
        ReadExample.readFileByChars(fileName);
        ReadExample.readFileByLines(fileName);
        ReadExample.readFileByRandomAccess(fileName);
    }
    /**
    ...
    public static void readFileByBytes(String fileName) {
        ...
        try {
            ...
            ReadExample.showAvailableBytes(in);            // 读入多个字节到字节数组中，byteread为一次读入的字节数
    ...
    public static void readFileByChars(String fileName) {
        ...
        try {
            ...
            int tempchar;
            while ((tempchar = reader.read()) != -1) {                // 对于windows下，\r\n这两个字符在一起时，表示一个换行。
                // 但如果这两个字符分开显示时，会换两次行。                // 因此，屏蔽掉\r，或者屏蔽\n。否则，将会多出很多空行。
                if (((char) tempchar) != '\r') {
